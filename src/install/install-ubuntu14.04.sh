...
mv ./dotcpp123/src/ /home/judge/src/
cd /home/judge/
...
chown www-data src/web/oj/upload data
chmod 777 -R src/web/oj/ueditor/php/upload
chown www-data src/web/oj/xktms/msg.txt
if grep client_max_body_size /etc/nginx/nginx.conf ; then 
	echo "client_max_body_size already added" ;
else
	sed -i "s:include /etc/nginx/mime.types;:client_max_body_size    1000m;\n\tinclude /etc/nginx/mime.types;:g" /etc/nginx/nginx.conf
fi
...
sed -i "s/post_max_size = 8M/post_max_size = 1000M/g" /etc/php5/fpm/php.ini
sed -i "s/upload_max_filesize = 2M/upload_max_filesize = 1000M/g" /etc/php5/fpm/php.ini
