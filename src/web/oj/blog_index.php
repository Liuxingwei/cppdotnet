...
while ($row=mysqli_fetch_object($result)){
    ...
    if ($row_hq[0]==1) {
        $pblog_data[$i][0]="<td><div class='list_box_index'><a target='_blank' class='list_title_index' href='/blog/".$row->blog_id.".html'><span style='color: red;font-weight: bold;border: 2px solid red;padding: 0px 3px;'>优质题解</span> ".$row->title."</a><br><br>";
    }
    else{
        $pblog_data[$i][0]="<td><div class='list_box_index'><a target='_blank' class='list_title_index' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br><br>";
    }

    $pblog_data[$i][1]="<span class='list_normal'>作者：<a href='/home/".$row->user_id."'>".getNickByid($row->user_id)."</a>　题号：".$row->problem_id."　语言：".$language."　编辑时间：".$row->post_time."</span>";
...
while ($row=mysqli_fetch_object($result)){
	...
	$ablog_data[$i][0]="<td><div class='list_box_index'><a target='_blank' class='list_title_index' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br><br>";
    
    $ablog_data[$i][1]="<span class='list_normal'>作者：<a href='/home/".$row->user_id."'>".getNickByid($row->user_id)."</a>　编辑时间：".$row->post_time."</span>";
...
while ($row=mysqli_fetch_object($result)){
    ...
    mysqli_free_result($result_hq);
    if ($row_hq[0]==1) {
        $blog_data_r[$i][0]="<td><a class='' target='_blank' href='/blog/".$row->blog_id.".html'><span style='color: red;font-weight: bold;border: 2px solid red;padding: 0px 3px;'>优质题解</span> ".$row->title."</a><br>";
    }
    else{
        $blog_data_r[$i][0]="<td><a class='' target='_blank' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br>";
    }
...
while ($row=mysqli_fetch_object($result)){
    ...
    mysqli_free_result($result_hq);
    if ($row_hq[0]==1) {
        $blog_data_ran[$i][0]="<td><a class='' target='_blank' href='/blog/".$row->blog_id.".html'><span style='color: red;font-weight: bold;border: 2px solid red;padding: 0px 3px;'>优质题解</span> ".$row->title."</a><br>";
    }
    else{
        $blog_data_ran[$i][0]="<td><a class='' target='_blank' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br>";
    }
...
while ($row=mysqli_fetch_object($result)){
    $blog_data_u[$i]=Array();
    $blog_data_u[$i]="";
    $blog_data_u[$i]="<td><span style='color: #999;font-weight: bold;'>".($i+1)."</span>　<a target='_blank' class='' style='display: inline-block;text-align: center;width: 85%;' href='/home/blog/".$row->user_id."'>".$row->nick."</a></td>";
    $i++;
}
mysqli_free_result($result);
