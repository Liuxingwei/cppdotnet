...
while ($row=mysqli_fetch_object($result)){
	...
	$blog_data[$i][0]="<td><div class='list_box_index'><a target='_blank' class='list_title_index' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br><br>";

    $blog_data[$i][1]="<span class='list_normal'>作者：<a href='/home/".$row->user_id."'>".getNickByid($row->user_id)."</a>　".$problem_id."　".$language."　编辑时间：".$row->post_time."</span>";
...
while ($row=mysqli_fetch_object($result)){
    ...
    $blog_data_ran[$i][0]="<td><a target='_blank' class='' href='/blog/".$row->blog_id.".html'>".$row->title."</a><br>";
