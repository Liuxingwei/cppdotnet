...
if(!isset($_SESSION['user_id']))if(isset($_SESSION['prev_page']))unset($_SESSION['prev_page']);
	...
	if (isset($_GET['cid'])){
		...
		//	print $cid;
			...
			if (!$contest_ok){
            			 $view_errors=  "这是个私有比赛, 输入密码才能查看. <br><br><a href=/oj/contestrank.php?cid=$cid>$MSG_WATCH_RANK</a>";
            			 $view_errors.=  "<br><br><form method=post action='contest$cid.html'>$MSG_CONTEST $MSG_PASSWORD:<input class=input-mini type=password name=password><input class='btn btn-primary' type=submit value='提交'></form>";
				require("template/".$OJ_TEMPLATE."/error.php");
				exit(0);
			}
		...
		order by pnum
			...
			while ($row=mysqli_fetch_object($result)){
				...
				$view_problemset[$cnt][2]= "<a target='_blank' href='/oj/contest".$cid."_problem".$cnt.".html'>$row->title</a>";
  ...
  //echo "$keyword";
	...
	if($ctype=="diy"){
		$sql_ctype=" AND `ctype`='diy' ";
	}
	else if($ctype=="main"){
		$sql_ctype=" AND `ctype`='main' ";
	}
	else if($ctype=="all"){
		$sql_ctype="";
	}
	...
	$sql="SELECT COUNT(`contest_id`) as cnt_id FROM `contest` WHERE `defunct`='N'".$sql_ctype;
	if (isset($_GET['last'])) {
		$sql="SELECT COUNT(`contest_id`) as cnt_id FROM `contest` WHERE `defunct`='N' AND `end_time`>'$now_date'";
	}

	$result=mysqli_query($mysqli,$sql);
	$row=mysqli_fetch_object($result);
	$cnt_contest=intval($row->cnt_id);
	$cnt_contest=intval($cnt_contest/$page_cnt);
	$view_total_page=$cnt_contest+1;
	//

	//选项卡分类
	$ctype_tab=$ctype;
	if ($ctype=="" && !isset($_GET['last'])) {
		$ctype_tab="main";
	}
	$divselected1=$divselected2="";
	if ($ctype_tab=="main") {
		$divselected1="div_selected";
	}
	else if ($ctype_tab=="diy") {
		$divselected2="div_selected";
	}
	else {
		$divselected1=$divselected2="";
	}
  ...
  $sql="SELECT * FROM `contest` WHERE `defunct`='N' ORDER BY `contest_id` DESC limit ".($page-1)*$page_cnt.",$page_cnt";
  $sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.defunct='N'".$sql_ctype." and contest.title like '%$keyword%'  order by contest_id desc limit ".($page-1)*$page_cnt.",$page_cnt";
  if (isset($_GET['last'])) {
  	$sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.end_time>'$now_date' and contest.defunct='N' and contest.title like '%$keyword%'  order by contest_id desc limit ".($page-1)*$page_cnt.",$page_cnt";
  }
			$result=mysqli_query($mysqli,$sql);
			...
			while ($row=mysqli_fetch_object($result)){
				...
				$view_contest[$i][1]= "<a href='/oj/contest$row->contest_id.html'>$row->title</a>";
				$start_time=strtotime($row->start_time);
				$end_time=strtotime($row->end_time);
			  		...
			  		if ($now>$end_time) {
			  			...
			  			if (isset($_SESSION['user_id'])) {
							...
							if (mysqli_num_rows($result_order) !==0) {
								...
								$view_contest[$i][3]= "<span style='color:blue'><a href='/oj/info_check.php?contest_id=".$row->contest_id."'>可预约</a></span>";
							}
			  			}
			  			else {
			  				$view_contest[$i][3]= "<span style='color:blue'><a href='/oj/info_check.php?contest_id=".$row->contest_id."'>可预约</a></span>";
