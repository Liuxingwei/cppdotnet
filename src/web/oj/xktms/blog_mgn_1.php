...
$sql="SELECT * FROM `blog` WHERE `status`=1 ORDER BY `post_time` DESC LIMIT $pstart,20 ";
$result=mysqli_query($mysqli, $sql);
...
<p><a href="blog_mgn.php">申精排序</a></p>

<div style="text-align:center;">
	<?php 
	for($i=1;$i<=$total_page;$i++){
	 	echo "<a href='blog_mgn_1.php?page=$i'>$i</a>";
	 }
	 ?>
</div>
<table class="table">
		...
		foreach($view_dis as $row){
		 ?>
		 	<tr>
		 		...
		 		<td><a target='_blank' href="/blog/<?php echo $row['blog_id'];?>.html"><?php echo $row['title']; ?></a></td>
