...
$sql="select `problem_id`,`title`,`in_date`,`defunct`,`vip` FROM `problem` where problem_id>=$pstart and problem_id<=$pend order by `problem_id` desc";
//echo $sql;
if($keyword) $sql="select `problem_id`,`title`,`in_date`,`defunct`,`vip` FROM `problem` where title like '%$keyword%' or source like '%$keyword%'";
...
if(isset($_SESSION['administrator'])) echo "<td>VIP";
...
for (;$row=mysqli_fetch_object($result);){
        ...
        if(isset($_SESSION['administrator'])){
            echo "<td><a href=problem_vip_change.php?id=$row->problem_id&getkey=".$_SESSION['getkey'].">"
            .($row->vip==0?"<span titlc='点击加入VIPproblem' class=green>ADD VIP</span>":"<span class=red title='点击移除VIPproblem'>RM VIP</span>")."</a></td>";
        }
        if(isset($_SESSION['administrator'])||isset($_SESSION['problem_editor'])){
                if(isset($_SESSION['administrator'])){
                ...
                if(isset($_SESSION['administrator'])||isset($_SESSION["p".$row->problem_id])||isset($_SESSION['problem_editor'])){
