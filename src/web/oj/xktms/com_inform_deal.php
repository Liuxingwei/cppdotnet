...
if(isset($_GET['ignore']) && isset($_GET['cid'])){
	require_once("../include/check_get_key.php");
	$cid=intval($_GET['cid']);
	$sql="UPDATE `comment` SET `status`=1,`inform_user_id`='NULL' WHERE `comment_id`='$cid'";
	mysqli_query($mysqli, $sql);	
}
require_once("../include/set_get_key.php");

$sql="SELECT COUNT(1) AS cnt FROM `comment` WHERE `status`=2";
...
$pstart=($page-1)*20;
//list
$sql="SELECT comment_id,comment.discuss_id,problem_id,comment.content,comment.user_id,comment.inform_user_id FROM `comment` INNER JOIN `discuss` WHERE comment.discuss_id=discuss.discuss_id AND comment.status=2 ORDER BY comment.post_time DESC LIMIT $pstart,20 ";
//$sql="SELECT * FROM `comment` WHERE `status`=2 ORDER BY `post_time` DESC LIMIT $pstart,20 ";
$result=mysqli_query($mysqli, $sql);
$cnt=0;
...
<div style="text-align:center;">
	...
	for($i=1;$i<=$total_page;$i++){
	 	echo "<a href='com_inform_deal.php?page=$i'>$i</a>";
	 }
	 ?>
</div>
<table class="table">
		...
		foreach($view_dis as $row){
		 ?>
		 	<tr>
		 		...
		 		<td>
		 			<a href="com_inform_deal.php?delete=1&getkey=<?php echo $_SESSION['getkey'] ?>&cid=<?php echo $row['cid']; ?>">删除</a>
		 			<a href="com_inform_deal.php?ignore=1&getkey=<?php echo $_SESSION['getkey'] ?>&cid=<?php echo $row['cid']; ?>">忽略</a>
		 		</td>
