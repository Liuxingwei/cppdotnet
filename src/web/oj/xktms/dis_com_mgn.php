...
$sql="SELECT COUNT(1) AS cnt FROM `discuss`";
...
$pstart=($page-1)*20;
//list
$sql="SELECT * FROM `discuss` ORDER BY `post_time` DESC LIMIT $pstart,20 ";
$result=mysqli_query($mysqli, $sql);
$cnt=0;
...
<div style="text-align:center;">
	...
	for($i=1;$i<=$total_page;$i++){
	 	echo "<a href='dis_com_mgn.php?page=$i'>$i</a>";
	 }
	 ?>
</div>
<table class="table">
		...
		foreach($view_dis as $row){
		 ?>
		 	<tr>
		 		...
		 		<td><a href="dis_com_mgn.php?delete=1&getkey=<?php echo $_SESSION['getkey'] ?>&did=<?php echo $row['did']; ?>">删除</a></td>
