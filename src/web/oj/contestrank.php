...
//order? 
$sql="SELECT `private`,`ctype` FROM `contest` WHERE `contest_id`=$cid";
$result=mysqli_query($mysqli,$sql);
$row=mysqli_fetch_array($result);
$private=$row[0];
$ctype=$row[1];
...
if($start_time>1489593600&&$private==0&&$ctype=="main") {
    $no_ord_mark=array();
    for ($i=0;$i<$user_cnt;$i++){
        $uid=$U[$i]->user_id;
        $sql="SELECT count(1) FROM `users` WHERE `user_id`='".$uid."' AND find_in_set('".$cid."',order_contest)";
        $result=mysqli_query($mysqli,$sql);
        $row_ord=mysqli_fetch_array($result);
        if ($row_ord[0]==0) {
            $no_ord_mark[]=0;
        } 
        else {
