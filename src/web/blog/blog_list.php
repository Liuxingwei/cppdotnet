...
while ($row=mysqli_fetch_object($result)){
    ...
    if ($row_hq[0]==1) {
        $blog_data[$i][0]="<td><a target='_blank' class='list_title' href='/".$row->user_id."/".$row->blog_id."'>　　<span style='color: red;font-weight: bold;border: 2px solid red;padding: 0px 3px;'>优质题解</span> ".$row->title."</a><br>";
    }
    else{
        $blog_data[$i][0]="<td><a target='_blank' class='list_title' href='/".$row->user_id."/".$row->blog_id."'>　　".$row->title."</a><br>";
    }
    if ($row->problem_id==0) {
        $blog_data[$i][1]="<span class='list_normal'>作者：<a href='/".$row->user_id."'>".getNickByid($row->user_id)."</a>　编辑时间：".$row->post_time."</span>";
    }
	else {
        $blog_data[$i][1]="<span class='list_normal'>作者：<a href='/".$row->user_id."'>".getNickByid($row->user_id)."</a>　题号：".$row->problem_id."　语言：".$language."　编辑时间：".$row->post_time."</span>";
    }
...
while ($row=mysqli_fetch_object($result)){
    ...
    mysqli_free_result($result_hq);
    if ($row_hq[0]==1) {
        $blog_data_r[$i][0]="<td><a target='_blank' class='' href='/".$row->user_id."/".$row->blog_id."'><span style='color: red;font-weight: bold;border: 2px solid red;padding: 0px 3px;'>优质题解</span> ".$row->title."</a><br>";
    }
    else {
