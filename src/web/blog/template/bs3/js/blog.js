function blog_scan(blog_id){
    $.ajax({
        'url':"/ajax_blog_scan.php?blog_id="+blog_id,
        'type':"GET"
    });
}
function home_scan(user_id){
    $.ajax({
        'url':"/ajax_blog_scan.php?user_id="+user_id,
        'type':"GET"
    });
}
function blog_del(blog_id)
{
    ...
    if (confirm_del==true)
        window.location = "/blog_del.php?blog_id="+blog_id;
}
function blog_nice(obj, blog_id){
    var obj = $(obj);
    $.ajax({
        'url':"/ajax_blog_nice.php?blog_id="+blog_id,
        'type':"GET",
...
function dis_nice(obj, discuss_id){
    var obj = $(obj);
    $.ajax({
        'url':"/ajax_blogdis_nice.php?discuss_id="+discuss_id,
        'type':"GET",
        'success':function(data){
            if(data.indexOf("true")!=-1){
                if(obj.html()=="赞"){
...
function comment_submit(ori,discuss_id){
    ...
    // console.log(userid);
    $.ajax({
        'url':"/ajax_blog_comment_submit.php",
        'type':"POST",
        'data':{
